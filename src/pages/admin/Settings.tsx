...
import { Settings, Lock, User, Clock, ShieldAlert, KeyRound, AlertCircle, RefreshCw, BarChart } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [statsRating, setStatsRating] = useState("5.0");
  const [statsReviews, setStatsReviews] = useState("200+");
  const [statsPackages, setStatsPackages] = useState("4+");
  const [statsCelebrations, setStatsCelebrations] = useState("1000+");
  const [isSavingStats, setIsSavingStats] = useState(false);
  const [statsSaveSuccess, setStatsSaveSuccess] = useState(false);
  ...
  useEffect(() => {
    ...
    // 3. Listen to website stats
    const statsUnsubscribe = onSnapshot(doc(db, "admin_settings", "website_stats"), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (data.rating) setStatsRating(data.rating);
        if (data.reviews) setStatsReviews(data.reviews);
        if (data.packages) setStatsPackages(data.packages);
        if (data.celebrations) setStatsCelebrations(data.celebrations);
      }
    });
  ...
  const handleSaveStats = async (e: React.FormEvent) => {
    ...
    setStatsSaveSuccess(false);
    try {
      await setDoc(doc(db, "admin_settings", "website_stats"), {
        rating: statsRating,
        reviews: statsReviews,
        packages: statsPackages,
        celebrations: statsCelebrations
      });
      setStatsSaveSuccess(true);
      setTimeout(() => setStatsSaveSuccess(false), 3000);
    } catch (err) {
      console.error("Error saving website stats:", err);
    } finally {
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
            ...
            <div className="glass-panel" style={{ padding: '2rem', height: 'fit-content' }}>
              ...
              <form onSubmit={handleSaveCredentials} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                ...
                <button 
                  ...
                  style={{ padding: '0.8rem', marginTop: '0.5rem', fontWeight: 'bold' }}
                >
            ...
            <div className="glass-panel" style={{ padding: '2rem', height: 'fit-content' }}>
              <h2 className="heading-luxury text-xl mb-6 text-[var(--accent-gold)]" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <BarChart size={20} /> Website Statistics
              </h2>
              ...
              <form onSubmit={handleSaveStats} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                {statsSaveSuccess && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(34,197,94,0.1)', color: '#22C55E', border: '1px solid rgba(34,197,94,0.2)', padding: '0.8rem', borderRadius: '6px', fontSize: '0.85rem' }}>
                    <AlertCircle size={16} />
                    <span>Live stats updated successfully!</span>
                  </div>
                )}

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 'bold' }}>Google Rating</label>
                  <input
                    type="text"
                    value={statsRating}
                    onChange={(e) => setStatsRating(e.target.value)}
                    style={{ width: '100%', padding: '0.6rem 1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', color: 'white', fontSize: '0.9rem', outline: 'none' }}
                  />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 'bold' }}>Happy Reviews</label>
                  <input
                    type="text"
                    value={statsReviews}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 'bold' }}>Premium Packages</label>
                  <input
                    type="text"
                    value={statsPackages}
                    onChange={(e) => setStatsPackages(e.target.value)}
                    style={{ width: '100%', padding: '0.6rem 1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', color: 'white', fontSize: '0.9rem', outline: 'none' }}
                  />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 'bold' }}>Successful Celebrations</label>
                  <input
                    type="text"
                    value={statsCelebrations}
                    onChange={(e) => setStatsCelebrations(e.target.value)}
                    style={{ width: '100%', padding: '0.6rem 1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', color: 'white', fontSize: '0.9rem', outline: 'none' }}
                  />
                </div>
                ...
                <button 
                  ...
                  style={{ padding: '0.8rem', marginTop: '0.5rem', fontWeight: 'bold' }}
                >
                  {isSavingStats ? "Saving..." : "Update Live Stats"}
                </button>
              </form>
            </div>
